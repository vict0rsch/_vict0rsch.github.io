...
I recently found my self in a tricky situation. Never had it been easier to save and restore a Tensorflow model than with `tf.saved_model.simple_save` and then `tf.saved_model.loader.load`. On the other hand, very little documentation exsists regarding the interaction with the Dataset API and how to restore a saved `tf.data.Dataset`'s `Iterator`.

This post contains standalone and deterministic code (at the end) to make it easily reproduceable for you. It runs under python 3 and Tensorflow 1.8.
